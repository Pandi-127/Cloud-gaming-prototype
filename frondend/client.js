...
class StreamClient {
    ...
    init() {
        ...
        this.elements.video.addEventListener('loadedmetadata', () => {
            ...
            this.log('success', `Video loaded: ${videoWidth}x${videoHeight}`);
        });
            ...
            try {
                ...
                stats.forEach(report => {
                    if (report.type === 'inbound-rtp' && report.kind === 'video') {
                        // Bitrate calculation
                        const now = Date.now();
                        const timeDiff = (now - this.stats.lastTime) / 1000;

                        if (timeDiff > 0 && report.bytesReceived) {
                            const bytesDiff = report.bytesReceived - this.stats.lastBytes;
                        ...
                        if (report.framesPerSecond) {
                            this.elements.fps.textContent = Math.round(report.framesPerSecond);
                        }

                        this.elements.packetsLost.textContent = report.packetsLost || 0;
